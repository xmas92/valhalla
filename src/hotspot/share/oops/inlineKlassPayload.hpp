...
private:
  ...
  static inline void copy(const PayloadA& src, const PayloadB& dst, LayoutKind copy_layout_kind, bool dest_is_initialized);

public:

  inline instanceOop make_private_buffer(TRAPS);
  ...
  template <typename OtherOopOrHandle>
  inline void copy_to_uninitialized(const InlineKlassPayloadImpl<OtherOopOrHandle>& dst);
