...
inline void RawAccessBarrier<decorators>::value_copy(const ValuePayload& src, const ValuePayload& dst) {
  ...
  AccessInternal::value_copy_internal(
      src.get_address(), dst.get_address(),
      static_cast<size_t>(klass->layout_size_in_bytes(copy_layout)));
}

template <DecoratorSet decorators>
inline void RawAccessBarrier<decorators>::value_store_null(const ValuePayload& dst) {
  AccessInternal::value_store_null(
      dst.get_address(),
      static_cast<size_t>(
