...
MetaObjArrayKlass* MetaObjArrayKlass::allocate_klass(ClassLoaderData* loader_data, int n,
                                       Klass* k, Symbol* name, ArrayKlass::ArrayProperties props,
                                       TRAPS) {
  assert(MetaObjArrayKlass::header_size() <= InstanceKlass::header_size(),
      "array klasses must be same size as InstanceKlass");

  int size = ArrayKlass::static_size(MetaObjArrayKlass::header_size());

  return new (loader_data, size, THREAD) MetaObjArrayKlass(n, k, name, Kind, props, ArrayKlass::is_null_restricted(props) ? markWord::null_free_array_prototype() : markWord::prototype());
}

Symbol* MetaObjArrayKlass::create_element_klass_array_name(JavaThread* current, Klass* element_klass) {
...
MetaObjArrayKlass* MetaObjArrayKlass::allocate_metaObjArray_klass(
    ClassLoaderData* loader_data, int n, Klass* element_klass, TRAPS) {
  ...
  MetaObjArrayKlass* oak = MetaObjArrayKlass::allocate_klass(loader_data, n, element_klass, name, ArrayProperties::INVALID, CHECK_NULL);
...
MetaObjArrayKlass::MetaObjArrayKlass(int n, Klass* element_klass, Symbol* name, KlassKind kind, ArrayKlass::ArrayProperties props, markWord mk) :
ArrayKlass(name, kind, props, mk), _refined_klasses() {
  set_dimension(n);
  set_element_klass(element_klass);
  set_properties(props);

  Klass* bk;
  if (element_klass->is_metaObjArray_klass()) {
    bk = MetaObjArrayKlass::cast(element_klass)->bottom_klass();
  } else {
    assert(!element_klass->is_objArray_klass(), "Only create arrays of Java Classes");
    bk = element_klass;
  }
  ...
  assert(is_metaObjArray_klass(), "sanity");
}

size_t MetaObjArrayKlass::oop_size(oop obj) const {
...
ArrayDescription MetaObjArrayKlass::array_layout_selection(Klass* element, ArrayProperties properties) {
...
ObjArrayKlass* MetaObjArrayKlass::allocate_klass_with_properties(ArrayKlass::ArrayProperties props, TRAPS) {
  ...
  ArrayDescription ad = MetaObjArrayKlass::array_layout_selection(element_klass(), props);
  switch (ad._kind) {
    case Klass::RefArrayKlassKind: {
      ak = RefArrayKlass::allocate_refArray_klass(this, props, CHECK_NULL);
      break;
    }
    case Klass::FlatArrayKlassKind: {
      assert(dimension() == 1, "Flat arrays can only be dimension 1 arrays");
      ak = FlatArrayKlass::allocate_flatArray_klass(this, props, ad._layout_kind, CHECK_NULL);
      break;
    }
    default:
      ShouldNotReachHere();
...
objArrayOop MetaObjArrayKlass::allocate_instance(int length, ArrayProperties props, TRAPS) {
  check_array_allocation_length(length, arrayOopDesc::max_array_length(T_OBJECT), CHECK_NULL);
  ObjArrayKlass* ak = klass_with_properties(props, THREAD);
  return ak->allocate_instance(length, THREAD);
}

oop MetaObjArrayKlass::multi_allocate(int rank, jint* sizes, TRAPS) {
  int length = *sizes;
...
bool MetaObjArrayKlass::can_be_primary_super_slow() const {
...
GrowableArray<Klass*>* MetaObjArrayKlass::compute_secondary_supers(int num_extra_slots,
...
void MetaObjArrayKlass::initialize(TRAPS) {
  bottom_klass()->initialize(THREAD);  // dispatches to either InstanceKlass or TypeArrayKlass
}

void MetaObjArrayKlass::metaspace_pointers_do(MetaspaceClosure* it) {
  ArrayKlass::metaspace_pointers_do(it);
  it->push(&_element_klass);
  it->push(&_bottom_klass);
  for (ObjArrayKlass* volatile& _refined_klass : _refined_klasses) {
    if (_refined_klass != nullptr) {
      it->push(&const_cast<ObjArrayKlass*&>(_refined_klass));
    }
  }
}

#if INCLUDE_CDS
void MetaObjArrayKlass::restore_unshareable_info(ClassLoaderData* loader_data, Handle protection_domain, TRAPS) {
  ArrayKlass::restore_unshareable_info(loader_data, protection_domain, CHECK);
  for (ObjArrayKlass* volatile& _refined_klass : _refined_klasses) {
    if (_refined_klass != nullptr) {
      _refined_klass->restore_unshareable_info(loader_data, protection_domain, CHECK);
    }
  }
}

void MetaObjArrayKlass::remove_unshareable_info() {
  ArrayKlass::remove_unshareable_info();

  for (ObjArrayKlass* volatile& _refined_klass : _refined_klasses) {
    if (_refined_klass != nullptr) {
      if (!CDSConfig::is_dumping_dynamic_archive()) {
        _refined_klass->remove_unshareable_info();
      } else {
        _refined_klass = nullptr;
      }
    }
  }
}

void MetaObjArrayKlass::remove_java_mirror() {
  ArrayKlass::remove_java_mirror();
  for (ObjArrayKlass* volatile& _refined_klass : _refined_klasses) {
    if (_refined_klass != nullptr && !CDSConfig::is_dumping_dynamic_archive()) {
      _refined_klass->remove_java_mirror();
    }
  }
}
#endif // INCLUDE_CDS

u2 MetaObjArrayKlass::compute_modifier_flags() const {
...
ModuleEntry* MetaObjArrayKlass::module() const {
  assert(bottom_klass() != nullptr, "ObjArrayKlass returned unexpected null bottom_klass");
  // The array is defined in the module of its bottom class
  return bottom_klass()->module();
}

PackageEntry* MetaObjArrayKlass::package() const {
  assert(bottom_klass() != nullptr, "ObjArrayKlass returned unexpected null bottom_klass");
  return bottom_klass()->package();
}

ObjArrayKlass* MetaObjArrayKlass::klass_with_properties(ArrayKlass::ArrayProperties props, TRAPS) {
  ObjArrayKlass* oak = AtomicAccess::load_acquire(&_refined_klasses[props]);

  if (oak == nullptr) {
    RecursiveLocker rl(MultiArray_lock, THREAD);
    oak = AtomicAccess::load(&_refined_klasses[props]);
    if (oak == nullptr) {
      if (AtomicAccess::load(&_refined_klasses[DEFAULT]) == nullptr) {
        // We must ensure that the DEFAULT klass is also created. (For some reason)
        ObjArrayKlass* default_oak = allocate_klass_with_properties(DEFAULT, CHECK_NULL);
        AtomicAccess::release_store(&_refined_klasses[DEFAULT], default_oak);
      }
      oak = allocate_klass_with_properties(props, CHECK_NULL);
      AtomicAccess::release_store(&_refined_klasses[props], oak);
    }
  }
...
void MetaObjArrayKlass::print_on(outputStream* st) const {
#ifndef PRODUCT
  Klass::print_on(st);
...
void MetaObjArrayKlass::print_value_on(outputStream* st) const {
...
void MetaObjArrayKlass::oop_print_on(oop obj, outputStream* st) {
  ShouldNotReachHere();
}

#endif //PRODUCT

void MetaObjArrayKlass::oop_print_value_on(oop obj, outputStream* st) {
  ShouldNotReachHere();
}

const char* MetaObjArrayKlass::internal_name() const {
...
void MetaObjArrayKlass::verify_on(outputStream* st) {
...
void MetaObjArrayKlass::oop_verify_on(oop obj, outputStream* st) {
