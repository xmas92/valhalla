...
inline bool InlineKlassPayloadImpl<OopOrHandle>::has_null_marker() const {
  ...
  return LayoutKindHelper::is_nullable_flat(_layout_kind) ||
         (_layout_kind == LayoutKind::BUFFERED &&
          _klass->has_nullable_atomic_layout());
}

template <typename OopOrHandle>
inline void InlineKlassPayloadImpl<OopOrHandle>::mark_as_non_null() {
  precond(has_null_marker());
  _klass->mark_payload_as_non_null(get_address());
}

template <typename OopOrHandle>
inline bool InlineKlassPayloadImpl<OopOrHandle>::is_marked_as_null() const {
  return has_null_marker() && _klass->is_payload_marked_as_null(get_address());
}
...
inline instanceOop InlineKlassPayloadImpl<Handle>::allocate_instance(TRAPS) const {
  return _klass->allocate_instance(THREAD);
}
...
inline instanceOop InlineKlassPayloadImpl<OopOrHandle>::read(TRAPS) {
  ...
  switch(_layout_kind) {
    ...
    case LayoutKind::NULL_FREE_NON_ATOMIC_FLAT: {
      ...
      InlineKlassPayload dst(res, _klass);
...
inline void InlineKlassPayloadImpl<OopOrHandle>::copy(const PayloadA& src, const PayloadB& dst, LayoutKind copy_layout_kind, bool dest_is_initialized) {
  ...
  const auto value_copy = [&] (const auto& src) {
    if (dest_is_initialized) {
      ...
      HeapAccess<IS_DEST_UNINITIALIZED>::value_copy(src.get_address(), dst.get_address(), klass, copy_layout_kind);
    }
  };
...
template <typename OopOrHandle>
template <typename OtherOopOrHandle>
inline void InlineKlassPayloadImpl<OopOrHandle>::copy_to_uninitialized(const InlineKlassPayloadImpl<OtherOopOrHandle>& dst) {
