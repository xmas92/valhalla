...
protected:
 ...
 public:
  ...
  oop obj_allocate(InstanceKlass* klass, size_t size, TRAPS);
  virtual oop array_allocate(ArrayKlass* klass, size_t size, int length, bool do_zero, TRAPS);
  oop class_allocate(InstanceMirrorKlass* klass, size_t size, TRAPS);
